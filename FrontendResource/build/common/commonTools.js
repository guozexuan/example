...
export function getQueryFromUrl(key) {
    ...
    search = search.replace(/\?/g, '&');    //兼容出现多个?的问题
...
export function convertObjectToSearch( obj ) {
    let query = '?';
    for( let key in obj ) {
...
var getScrollOffsets = function getScrollOffsets(_w) {//获取页面的滚动位置
    _w = _w || window;
    //for all and IE9+
...
var getViewPortSize = function getViewPortSize(_w) {//获取页面的窗口大小
    _w = _w || window;
    //for all and IE9+
...
export function scrollPosition(_obj) {//参数_obj可以是任何页面上存在的元素的id，或者是指定元素本身
    var targetX, targetY;
...
export function hideMenuAndFooterForXinDongQiMingApp() {
    let ua = window.navigator.userAgent.toLowerCase();
    return /linghit xindongqiming/.test( ua );
}
